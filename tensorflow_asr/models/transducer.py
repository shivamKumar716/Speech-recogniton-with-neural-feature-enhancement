...
class Transducer(Model):
    ...
    def _build(self, input_shape):
        ...
        ep = tf.Variable(0, trainable=False, dtype=tf.int64)        #added st please cmnt
        self([inputs, input_length, pred, pred_length,ep], training=False)
    ...
    def call(self, inputs, training=False, **kwargs):
        ...
        """
        # print(kwargs['ep'])
        features, _, prediction, prediction_length,ep = inputs
        kwargs.update(ep=ep)
        enc, mse_loss = self.encoder(features, training=training,**kwargs)
